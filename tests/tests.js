...
(function($) {
	...
	test('if an action has data-overflow=true, it is placed in the action overflow menu', function () {
		var actionBarArea = $("#action-bar-area5");
		equal(actionBarArea.find( "[data-role='action']" ).length, 2);
	});

	test('if an tab has data-overflow=true, it is placed in the tab overflow menu', function () {
		var actionBarArea = $("#action-bar-area6");
		equal(actionBarArea.find( "[data-role='tab']" ).length, 2);
	});
